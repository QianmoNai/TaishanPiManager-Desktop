...
本地 v2.67 新增独立 ADB 服务端口 `127.0.0.1:5038`，不再共用手机助手默认的 `5037`。覆盖设备扫描、文件操作、终端和各类数据流；不自动停止其他软件。独立端口不能完全避免 USB 抢占，详见[使用说明](docs/使用说明.md)。v2.67 尚未发布；GitHub 已发布 v2.66-release。

侧栏点击“检查更新”，后台查询 Gitee 最新正式 Release，直接显示简短文字，不弹出独立窗口。发现新版时点击“前往发布页”查看说明并自行下载。无需连接开发板，不上传设备信息，不自动安装。普通 Git 标签不是更新来源；网络失败、限流或仓库不可公开访问时会明确提示，不能视为“已经是最新版”。详见[使用说明](docs/使用说明.md)。
...
项目保留了从 `v1.0` 起的迭代提交历史，当前本地版本基线标签已到 `v2.67`（尚未推送）。查看历史可进入 [Gitee Tags](https://gitee.com/qianmonai/TaishanPiManager-Desktop/tags) 或 [GitHub Tags](https://github.com/QianmoNai/TaishanPiManager-Desktop/tags)；已编写的版本验证记录保存在 [`docs/`](docs/) 中，并非每个标签都有独立验证文档。
