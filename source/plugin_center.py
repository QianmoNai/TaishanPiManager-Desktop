...
class PluginCenter(QWidget):
    def __init__(self, owner, label, button, card, icon):
        ...
        self.category='全部'; self.detail_callback=None
        ...
        self.summary=label('1 款可安装插件 · 4 项内置工具','heroCaption'); box.addWidget(self.summary); layout.addWidget(hero)
        ...
        items=[
            ...
            ('wifi','Wi-Fi 管理','扫描附近网络，选择热点并连接泰山派。','网络','wifi','#30b86b','内置工具 · 无需安装',5),
        ...
        for key,title,description,category,symbol,color,meta,page in items:
            ...
            badge=label('待检测' if key=='traffic' else '内置','badge'); row.addWidget(badge); box.addLayout(row)
            box.addWidget(label(description,'subtle',True)); box.addStretch()
            row=QHBoxLayout(); row.addWidget(label(category+' · '+('板端插件' if key=='traffic' else '桌面工具'),'caption')); row.addStretch()
            open_btn=button('查看详情' if key=='traffic' else '打开',lambda checked=False,p=page:self.open_traffic() if p is None else owner.go(p),'primary' if key=='traffic' else 'secondary')
        ...
        footer=label('本地插件目录 · 当前提供 1 款可安装插件，其余为内置工具快捷入口。','caption',True); layout.addWidget(footer)
