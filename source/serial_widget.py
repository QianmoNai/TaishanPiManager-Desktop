...
from PySide6.QtWidgets import QWidget,QVBoxLayout,QHBoxLayout,QGridLayout,QPlainTextEdit,QLineEdit,QCheckBox,QSpinBox,QFileDialog,QApplication
...
class SerialPanel(QWidget):
    def __init__(self,owner,label,button,card):
        super().__init__(); self.owner=owner; self.generation=0; self.connected=False; self.session_epoch=0; self.preparing=False; self.closing=False; self.rx=0; self.tx=0; self.pending=bytearray(); self.log_pending=bytearray(); self.info={}; self.decoder=WaveDecoder(); self.text_decoder=codecs.getincrementaldecoder('utf-8')('replace')
        ...
        row=QHBoxLayout(); self.port=QComboBox(); self.port.setMinimumWidth(170); row.addWidget(self.port,1); self.refresh_btn=button('刷新串口',self.refresh); row.addWidget(self.refresh_btn); self.open_btn=button('打开串口',self.open_port,'primary'); row.addWidget(self.open_btn); row.addWidget(button('关闭串口',self.stop)); inner.addLayout(row)
        ...
        for i,(title,widget) in enumerate([('波特率',self.baud),('数据位',self.bits),('校验',self.parity),('停止位',self.stops),('流控',self.flow)]): grid.addWidget(label(title,'caption'),0,i); grid.addWidget(widget,1,i)
        ...
        frame,inner=card(); row=QHBoxLayout(); row.addWidget(label('数据接收','section')); row.addStretch(); self.view=QComboBox(); self.view.addItems(['UTF-8 文本','HEX']); row.addWidget(self.view); self.pause=QCheckBox('暂停显示'); row.addWidget(self.pause); self.follow=QCheckBox('自动滚动'); self.follow.setChecked(True); row.addWidget(self.follow); row.addWidget(button('清空',self.clear)); row.addWidget(button('保存日志',self.save_log)); inner.addLayout(row)
